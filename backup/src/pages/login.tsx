...
import { LoginFlow, PublicApi as KratosPublicApi } from '@oryd/kratos-client';
  ...
  return (
    <>
      ...
      >
        ...
        {sorted_methods.map((method, index) => <KratosForm key={index} config={method.config} method={method.method} divider={index + 1 < sorted_methods.length} />)}
      </FormWrapper>
      ...
      <Grid container>
        ...
        <Grid item>
          <Link href={`/self-service/registration/browser?return_to=${encodeURIComponent(props.return_to)}`} variant="body2">
